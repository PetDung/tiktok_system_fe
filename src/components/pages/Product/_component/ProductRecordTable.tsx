    ...
    <>
      ...
      >
        <table className="min-w-full text-xs table-auto">
          <thead className="bg-gray-300 sticky top-0 z-20 text-left">
            <tr>
              ...
              <th className="px-2 py-2 w-36">Images</th>
              <th className="px-2 py-2 max-w-[200px]">Product</th>
              <th className="px-2 py-2 max-w-[200px]">Category</th>
              <th className="px-2 py-2 w-28">Status</th>
              <th className="px-2 py-2 w-28">Reason</th>
              <th className="px-2 py-2 w-32">Link</th>
            </tr>
          </thead>
            ...
            {products.map((product, index) => (
              <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-2 py-2 text-gray-600">
                  ...
                  <p className="font-bold">{product.shop.userShopName}</p>
                  <div><span className="text-xs"></span> {formatDate(product.createTime)}</div>
                </td>
                ...
                <td className="px-2 py-2 text-center">
                  {product.auditFailedReasons?.length ? (
                    <button
                      onClick={() => openReasonModal(product.auditFailedReasons)}
                      className="flex items-center gap-1 text-sm text-blue-500 hover:underline mx-auto"
                    >
                      <Info size={14} /> View
                    </button>
                  ) : (
                ...
                <td className="px-2 py-2 text-center">
                  <div className="flex items-center justify-center gap-2">
                    <a href={`https://shop.tiktok.com/view/product/${product.id}`} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">Open</a>
                    <button onClick={() => handleCopy(`https://shop.tiktok.com/view/product/${product.id}`, product.id)} className="text-gray-400 hover:text-gray-600 transition" title="Copy link">
                      <Copy size={16} />
                    </button>
